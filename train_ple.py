...
def data_preparation():
    ...
    test_data = transformed_other.iloc[test_indices].astype(np.float32)
    test_label = [dict_other_labels[key][test_indices] for key in sorted(dict_other_labels.keys())]
    train_data = transformed_train.astype(np.float32)
...
def main():
    ...
    ctr_logit_, cvr_logit_  = Mmof_3loss.get_mmoe_model(input_layer, extra_input=None,
                                                                         training=None, params=Mmof_3loss.params)

    ctr_logits = Dense(2, activation=None, use_bias=True,name=output_info[0][1])(ctr_logit_)
    cvr_logits = Dense(2, activation=None, use_bias=True,name=output_info[1][1])(cvr_logit_)
    #ctr_nocvr_logit = Dense(1, activation=None, use_bias=True)(ctr_nocvr_logit)
    output_layers=[ctr_logits,cvr_logits]
    #print('train_label data shape = {}'.format(train_label.))
    print('ctr_logits data shape = {}'.format(ctr_logits.shape))
    print('',)
    # Compile model
